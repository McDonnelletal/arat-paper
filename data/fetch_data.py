...
def fetch_diabetes():
    ...
    raw_file = DIABETES_DIR / "diabetic_data.csv"

    if train_file.exists() and test_file.exists() and raw_file.exists():
        print(f"  Diabetes already present: {train_file.stat().st_size:,} bytes")
        return
    if train_file.exists() and test_file.exists():
        print("  Diabetes split bundled; fetching the raw diabetic_data.csv for reference...")
    ...
    # The pre-encoded 21-feature split (diabetes_train/test.csv) is bundled in the
    # repo (UCI 296 is CC BY); this also fetches the raw diabetic_data.csv for reference.
    print("  Diabetes: raw diabetic_data.csv fetched; the pre-encoded 21-feature split")
    print("    (diabetes_train/test.csv) is bundled in the repo (see data/README.md).")
