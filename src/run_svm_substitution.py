...
def run_unsw():
    ...
    if not (DATA_DIR / "UNSW_NB15_training-set.csv").exists():
        print("  UNSW-NB15 data not found in data/unsw_nb15/ -- skipping the UNSW arm.")
        print("  Run 'python data/fetch_data.py' (see data/README.md); the Diabetes arm")
        print("  runs on the bundled split regardless.")
        return None
...
def run_diabetes():
    ...
    # Train SVM-RBF on the FULL training set (no subsample). A subsample
    # materially understates RF/SVM error convergence on Diabetes (phi,
    # disagreement); full-data training is required to reproduce the paper's
    # Diabetes-SVM result. SVM-RBF is O(n^2); ~15 min on a 4-core laptop at ~81k.
    print(f"  Training SVM-RBF (C=10, full n={len(y_train):,})...")
    svm = SVC(kernel="rbf", C=10.0, gamma="scale", class_weight="balanced",
              random_state=SEED, probability=True, cache_size=1000)
