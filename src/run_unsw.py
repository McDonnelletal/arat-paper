...
Reproduces: Table 1 (routing strategies), the Sec. 6.2 error-dependence
stats, the Sec. 6.3 escalation router and θ sweep, the confusion matrix
figure (Fig. 2, Sec. 6.4), all confidence intervals, and the
...
def main(mode="all"):
    ...
    # --- Routing strategies ---
    # Paper Sec. 4.3 describes arbitration via a soft-disagreement score
    # c = 0.5*H(p_merged) + 0.5*d (both min-max scaled). The deterministic layers
    # below realise that score's two components directly rather than thresholding
    # c itself: the d component fires the conservative override (any disagreement
    # -> more severe prediction), and the H component drives the unanimous-Normal
    # safety flag (agreement on class 0 with merged entropy > theta). Queue
    # ordering (the escalation_score of Fig. 1) is provided by the held-out
    # XGBoost router further below.
    ...
    if mode in ("all", "tables"):
        ...
        # --- Error dependence (Sec. 6.2 of the paper) ---
        print("\n=== ERROR DEPENDENCE (Sec. 6.2) ===")
        ...
        # --- Escalation router (held-out validation protocol) ---
        # The router is trained on a held-out validation split (carved from TRAIN) and
        # scored on the official test set, so its recall and AUROC are strictly
        # out-of-sample. Routing features (13 total): both agents' class-probability
        # vectors, the Jensen-Shannon divergence between them, each agent's Shannon
        # entropy, and each agent's top-class confidence. "Dangerous" = both agents
        # AGREE on an under-prediction -- the failures disagreement-based escalation
        # cannot catch. Reported as the mean over 3 seeds.
        print("\n=== ESCALATION ROUTER (held-out validation) ===")

        def _router_feats(pa, pb):
            eps = 1e-10
            js = np.array([jensenshannon(p, q) for p, q in zip(pa, pb)]).reshape(-1, 1)
            ea = (-np.sum(pa * np.log(pa + eps), axis=1)).reshape(-1, 1)
            eb = (-np.sum(pb * np.log(pb + eps), axis=1)).reshape(-1, 1)
            return np.nan_to_num(np.hstack([pa, pb, js, ea, eb,
                                            pa.max(1).reshape(-1, 1), pb.max(1).reshape(-1, 1)]))

        def _danger(pa_pred, pb_pred, yy):
            return (pa_pred == pb_pred) & (pa_pred < yy) & (yy >= 1)

        # Recall is reported at the paper's two review budgets (6.79% = the safety
        # flag's natural rate, and 15%) plus 20% for reference. The Table 4 ARAT
        # row additionally needs the FULL-ROUTING residual under-prediction at the
        # 15% budget: conservative override applied, safety-flag cases mandatorily
        # escalated, the remaining budget filled by router score, and escalated
        # cases assumed resolved (the same "Under" semantics as the UQ baselines).
        esc_auroc, esc_rec = [], {0.0679: [], 0.15: [], 0.20: []}
        esc_under15 = []
        for esc_seed in (42, 123, 256):
            Xa, Xv, ya, yv = train_test_split(X_train, y_train, test_size=0.25,
                                              random_state=esc_seed, stratify=y_train)
            rf_e = RandomForestClassifier(n_estimators=500, min_samples_leaf=2,
                                          class_weight="balanced", random_state=esc_seed,
                                          n_jobs=-1).fit(Xa, ya)
            knn_e = KNeighborsClassifier(n_neighbors=5, n_jobs=-1).fit(Xa, ya)
            pav, pbv = rf_e.predict_proba(Xv), knn_e.predict_proba(Xv)
            dv = _danger(pav.argmax(1), pbv.argmax(1), yv).astype(int)
            router = XGBClassifier(n_estimators=300, eval_metric="logloss", verbosity=0,
                                   random_state=esc_seed, n_jobs=-1).fit(_router_feats(pav, pbv), dv)
            pat, pbt = rf_e.predict_proba(X_test), knn_e.predict_proba(X_test)
            dt = _danger(pat.argmax(1), pbt.argmax(1), y_test)
            sc_t = router.predict_proba(_router_feats(pat, pbt))[:, 1]
            esc_auroc.append(roc_auc_score(dt, sc_t))
            order = np.argsort(-sc_t)
            for b in (0.0679, 0.15, 0.20):
                flag_b = np.zeros(N, dtype=bool); flag_b[order[: int(b * N)]] = True
                esc_rec[b].append(float((flag_b & dt).sum() / dt.sum()))

            # Full-routing residual under @ 15% for this seed's agent pair
            rf_p_e, knn_p_e = pat.argmax(1).astype(int), pbt.argmax(1).astype(int)
            ens_e = (pat + pbt) / 2.0
            pred_e = ens_e.argmax(1).astype(int)
            dis_e = rf_p_e != knn_p_e
            pred_e[dis_e] = np.maximum(rf_p_e, knn_p_e)[dis_e]
            flag_e = (~dis_e) & (pred_e == 0) & (sp_entropy(ens_e.T, base=2) > THETA)
            under_e = pred_e < y_test
            esc_set = flag_e.copy()
            room = int(0.15 * N) - int(esc_set.sum())
            if room > 0:
                for i in order:
                    if not esc_set[i]:
                        esc_set[i] = True; room -= 1
                        if room == 0:
                            break
            esc_under15.append(float((under_e & ~esc_set).sum() / N))
        escalation = {
            "protocol": "held-out validation split; XGBoost router; 13 features; "
                        "dangerous = both agents agree & under-predict; mean of 3 seeds",
            "n_features": 13,
            "auroc_heldout_mean": round(float(np.mean(esc_auroc)), 4),
            "recall_at_679pct_mean": round(float(np.mean(esc_rec[0.0679])), 4),
            "recall_at_679pct_std": round(float(np.std(esc_rec[0.0679])), 4),
            "recall_at_15pct_mean": round(float(np.mean(esc_rec[0.15])), 4),
            "recall_at_15pct_std": round(float(np.std(esc_rec[0.15])), 4),
            "recall_at_20pct_mean": round(float(np.mean(esc_rec[0.20])), 4),
            "under_at_15pct_full_routing_mean": round(float(np.mean(esc_under15)), 4),
            "under_at_15pct_full_routing_std": round(float(np.std(esc_under15)), 4),
        }
        print(f"  held-out AUROC      : {escalation['auroc_heldout_mean']:.4f}")
        print(f"  recall@6.79% (3-seed): {escalation['recall_at_679pct_mean']:.4f} "
              f"+/- {escalation['recall_at_679pct_std']:.4f}")
        print(f"  recall@15% (3-seed) : {escalation['recall_at_15pct_mean']:.4f} "
              f"+/- {escalation['recall_at_15pct_std']:.4f}")
        print(f"  recall@20% (3-seed) : {escalation['recall_at_20pct_mean']:.4f}")
        print(f"  under@15% full routing (3-seed): "
              f"{escalation['under_at_15pct_full_routing_mean']:.4f} "
              f"+/- {escalation['under_at_15pct_full_routing_std']:.4f}")
        ...
        all_results = {"dataset": {"train_n": len(y_train), "test_n": N, "features": len(selected)},
                       "table1": t1, "table2": t2, "escalation": escalation,
