...
Reproduces the Diabetes cross-dataset validation (Sec. 6.5 of the paper);
the v2 values feed Table 2's Diabetes columns.
...
Pipeline:
    ...
    4. Held-out XGBoost escalation router (identical protocol to run_unsw.py:
       13 routing features, validation split carved from TRAIN, 3 seeds)
...
def main():
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ESCALATION ROUTER (held-out validation, identical protocol to run_unsw.py)
    # ═══════════════════════════════════════════════════════════════════════════
    # XGBoost router on 13 routing-time features (both agents' class-probability
    # vectors, Jensen-Shannon divergence, each agent's entropy, each agent's
    # top-class confidence), trained on a validation split carved from TRAIN and
    # scored on the held-out test set; "dangerous" = both agents agree on an
    # under-prediction. Mean of 3 seeds. The paper reports no Diabetes router
    # number; this shows the same held-out router applies here (Sec. 6.5).
    print(f"\n{'='*70}")
    print("ESCALATION ROUTER (held-out validation)")
    print(f"{'='*70}")
    ...
    def _danger(pa_pred, pb_pred, yy):
        return (pa_pred == pb_pred) & (pa_pred < yy)
    ...
    for esc_seed in (42, 123, 256):
        ...
        rf_e = RandomForestClassifier(n_estimators=500, max_depth=15, min_samples_leaf=5,
        ...
        top = np.zeros(N, dtype=bool); top[order[: int(0.15 * N)]] = True
        esc_rec15.append(float((top & dt).sum() / max(dt.sum(), 1)))
        del rf_e, knn_e, router; gc.collect()
    escalation = {
        ...
        "recall_at_15pct_mean": round(float(np.mean(esc_rec15)), 4),
        "recall_at_15pct_std": round(float(np.std(esc_rec15)), 4),
    }
    ...
    # Joint-error inflation CI (paper Sec. 6.5: 1.30x, 95% CI [1.28, 1.31]) --
    # percentile bootstrap over test cases
    rng_inf = np.random.RandomState(SEED)
    inf_boot = []
    for _ in range(1000):
        idx = rng_inf.randint(0, N, N)
        rw, kw = rf_wrong_ci[idx], knn_wrong_ci[idx]
        d_inf = rw.mean() * kw.mean()
        inf_boot.append((rw & kw).mean() / d_inf if d_inf > 0 else 0)
    inf_lo, inf_hi = np.percentile(inf_boot, [2.5, 97.5])
    ci["inflation_ci95"] = (round(float(inf_lo), 4), round(float(inf_hi), 4))
    print(f"  inflation 95% CI: [{inf_lo:.4f}, {inf_hi:.4f}]")
    ...
    print(f"  Router AUROC (held-out):     {escalation['auroc_heldout_mean']:.4f}")
